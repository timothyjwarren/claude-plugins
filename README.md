...
| [git-cli-permissions](plugins/git-cli-permissions/) | Auto-approve safe, local-only `git`/`gh` commands (status, diff, add, commit, fetch, pull, `gh pr view`, etc.); defer everything else to normal permission settings. |
...
/plugin install git-cli-permissions
