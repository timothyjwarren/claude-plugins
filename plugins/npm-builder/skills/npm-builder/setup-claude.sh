...
GUARD_SCRIPT="$SCRIPT_DIR/npm-build-guard.sh"

if [ ! -f "$GUARD_SCRIPT" ]; then
  echo "ERROR: npm-build-guard.sh not found at $GUARD_SCRIPT" >&2
  exit 1
fi
...
echo "  $SCRIPT_DIR/run-for-agent.sh install"
echo ""
echo "To build:"
echo "  $SCRIPT_DIR/run-for-agent.sh run build"
